...
* BREAKING CHANGE: We've greatly improved the performance of the `force:source:push|pull` commands.  (more stuff) However, as a result of this improvement, we've changed the Salesforce CLI-specific hooks that plug-in developers use to customize the behavior of Salesforce CLI commands. In particular, we no longer support the XX hooks and we've changed the result types for these hooks:

    * XX (what did we do)
    * YY (what did we do)

    The [Hooks topic of the Salesforce CLI Plug-In Developer Guide](https://developer.salesforce.com/docs/atlas.en-us.sfdx_cli_plugins.meta/sfdx_cli_plugins/cli_plugins_customize_hooks_list.htm) will soon be updated with complete information about the changed hooks and result types.
